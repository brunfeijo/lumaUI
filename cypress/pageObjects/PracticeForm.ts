...
export class PracticeFormPage {
  ...
  selectGender(v: Gender): void {
    ...
    cy.contains("label.custom-control-label", v)
      .should("be.visible")
      .click({ force: true });
  }
  ...
  checkHobby(label: "Sports" | "Reading" | "Music"): void {
    ...
    cy.contains("label.custom-control-label", label)
      .should("be.visible")
      .click({ force: true });
  }
  ...
  verifySubmissionMatchesSnapshot(): void {
    ...
    const stateCityRegex = new RegExp(
      `${this.snapshot.state}.*${this.snapshot.city}`
    );
